...
var FETCH = 'fetch';
var ERROR = 'error';
var HTTP_SUCCESS$2 = 1;
var HTTP_ERROR$2 = 2;
var PERFORMANCE = 'performance';
var REACT_ERROR = 'reactError';
var VUE_ERROR = 'vueError';
/**
 * 上报错误类型
 */
...
function errorBoundaryReport(ex) {
  ...
  var result = extractErrorStack(ex);
  result.type = REACT_ERROR;
  var data = {
    simpleUrl: getLocationHref(),
...
function handleVueError(monitor, err, vm, info, Vue) {
  ...
  if (variableTypeDetection.isString(version)) {
    ...
    switch (getBigVersion(version)) {
      case 2:
        data = _objectSpread2(_objectSpread2({}, errorMessage), vue2VmHandler(vm));
        break;

      case 3:
        data = _objectSpread2(_objectSpread2({}, errorMessage), vue3VmHandler(vm));
        break;
  ...
  if (!monitor) return;
  monitor.logSave(JS_ERROR, param);
}
/**
 * vue 2
 * @param {*} vm 
...
function vue2VmHandler(vm) {
  ...
  if (vm.$root === vm) {
    ...
    componentName = (name ? 'component <' + name + '>' : 'anonymous component') + (vm._isVue && vm.$options && vm.$options.__file ? ' at ' + (vm.$options && vm.$options.__file) : '');
  }
...
var MitoVue = {
  ...
  install: function install(Vue) {
    ...
    var that = this; // vue 提供 warnHandler errorHandler报错信息
...
var HandleEvents = {
   ...
   * @param {*} data 
   */
  handleHttp: function handleHttp(data) {
    ...
    if (!!data && data.status != 200) {
      param = {
        simpleUrl: getLocationHref(),
        httpUrl: data.url || "",
        httpUploadType: HTTP_ERROR$2,
        responseText: JSON.stringify(data.responseText || ""),
        httpStatus: data.status || 0
      };

      if (!!param && !!url && url != "".concat(vm.queue.baseUrl).concat(vm.queue.api)) {
        vm.logSave(HTTP_LOG, param);
      }
    }
   ...
   * @param {*} data
   */
  handleHashchange: function handleHashchange(data) {
    this.savePv(data);
  },
  ...
  savePv: function savePv(data) {
    try {
      ...
      var _parseUrlToObj = parseUrlToObj(from),
          parsedFrom = _parseUrlToObj.relative;

      var _parseUrlToObj2 = parseUrlToObj(to),
          parsedTo = _parseUrlToObj2.relative;

      var param = {
        simpleUrl: parsedFrom ? parsedFrom : '/',
        referrer: parsedTo ? parsedTo : '/'
      };
      vm.logSave(PAGE, param);
    } catch (e) {}
  },
  ...
  handleUnhandleRejection: function handleUnhandleRejection(ev) {
    ...
    if (isError(ev.reason)) {
      data = _objectSpread2(_objectSpread2({}, data), extractErrorStack(ev.reason));
    }
    ...
    return function () {
      ...
      on(this, 'loadend', function () {
        try {
          ...
          var eTime = getTimestamp$1();
          this.mito_xhr.time = eTime;
          this.mito_xhr.status = status;

          if (['', 'json', 'text'].indexOf(responseType) !== -1) {
            this.mito_xhr.responseText = _typeof(response) === 'object' ? JSON.stringify(response) : response;
          }
...
function replaceFetch() {
  ...
  replaceOld(window, FETCH, function (originalFetch) {
    return function (url, config) {
      try {
        var sTime = getTimestamp$1();
        ...
        var headers = new Headers(config && config.headers || {});
        ...
        return originalFetch.apply(window, [url, config]).then(function (res) {
          var tempRes = res.clone();
          var eTime = getTimestamp$1();
          handlerData = _objectSpread2(_objectSpread2({}, handlerData), {}, {
